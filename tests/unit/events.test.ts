...
describe("rule engine and event ledger", () => {
  ...
  it("runs only registered data stages and advances the authoritative clock", () => {
    ...
    expect(listSimulationStages().map((stage) => stage.id)).toEqual(["environment", "ecology", "agents", "culture", "society", "lod"]);
  ...
  it("keeps default stages when an extension stage is registered", () => {
    ...
    expect(listSimulationStages().map((stage) => stage.id)).toEqual(["environment", "ecology", "agents", "extension", "culture", "society", "lod"]);
  });
});
