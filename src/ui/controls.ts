...
export type TimeControlOptions = { onReset?: () => void };

export const bindTimeControls = (root: ParentNode, client: WorkerClient, options: TimeControlOptions = {}): void => {
  ...
  root.querySelector<HTMLButtonElement>("#reset-button")?.addEventListener("click", () => { options.onReset?.(); client.send({ type: "reset" }); });
