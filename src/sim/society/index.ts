...
export { facilityEffectProfileForRegion, facilityEffectProfilesForState, facilityIdFor, facilityOperationalEffect, facilityWorkforceRequiredFor, stepFacilities } from "./facilities.ts";
export { compactOrganizationRecords, MAX_CHILD_ORGANIZATION_IDS, MAX_DIPLOMATIC_RELATIONS, MAX_ORGANIZATION_RECORDS, MAX_ORGANIZATION_TERRITORY_REGIONS, MAX_ORGANIZATIONS_PER_SUMMARY } from "./archive.ts";
...
const eventFromDraft = (draft: WorldEventDraft, state: Parameters<SimulationStage["run"]>[0], input: Parameters<SimulationStage["run"]>[1], index: number): WorldEvent => ({
  id: `transient:${draft.ruleId}:${state.tick + 1}:${index}`,
  tick: state.tick + 1,
  ...(draft.years === undefined ? { years: state.years + input.elapsedYears } : { years: draft.years }),
  kind: draft.kind,
  ruleId: draft.ruleId,
